...
function final(someInput: string, callback: (sentence: string) => string) {
  callback(`${someInput} and terminated by executing callback `);
}

function middleware(someInput: string, callback: (sentence: string) => string) {
  return final(`${someInput} touched by middleware `, callback);
}

function initiate() {
  const someInput = "hello this is a function ";
  middleware(someInput, (result: string) => {
    console.log(result);
    return result;
  });
}
